...
def features_extract(dataset, teams):	
	...
	for i in range(0, len(dataset)):
		...
		win_flag = dataset[i, 8]
		moneyline = dataset[i, 10]
		opponent_moneyline = dataset[i, 11]
		dataset[i, 10] = american_to_decimal(dataset[i, 10])
		dataset[i, 11] = american_to_decimal(dataset[i, 11])
...
def create_dataset(data):
	...
	dataX = numpy.delete(data, [1, 2, 3, 4], axis = 1)
...
def get_percentage(data):
	...
	for i in range(0, len(data)):
		...
		if temp_flag == numpy.int_(data[i, 8]):
			percent3 = percent3 + 1
...
def load_dataset():
	return pandas.read_csv(
		'seeds/inputs.csv',
		usecols = [1, 3, 4, 7, 8, 9, 10, 11, 13, 14, 15, 16, 19, 20],
		engine = 'python'
	).values
...
def model_predict(model, test_x):
	...
	return model.predict(test_x, batch_size=1, verbose=1)
...
for select_date in range(start_date, end_date):	
	...
	testPredict = model_predict(model, testX)
