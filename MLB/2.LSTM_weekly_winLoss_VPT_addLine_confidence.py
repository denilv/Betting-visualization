...
def get_confidence(d_set, testPredict, select_date):
	...
	for i in range(trainLength, testLength + 1):
		...
		print d_set[i, 4], d_set[i, 5], str(team1_pos)+'/'+str(team1_nav), str(team2_pos)+'/'+str(team2_nav)
		win_loss.append(x)
		k += 1
...
def build_model(input_dim, output_dim):
	...
	model.add(LSTM(input_dim=input_dim, output_dim=800, return_sequences=True))
	model.add(LSTM(input_dim=800, output_dim=1000, return_sequences=True))
	model.add(Dropout(0.2))
	model.add(LSTM(input_dim=1000, output_dim=500, return_sequences=False))
	model.add(Dropout(0.2))
	model.add(Dense(input_dim=500, output_dim=output_dim))
